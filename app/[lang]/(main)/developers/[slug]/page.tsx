...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> {
  ...
  const title = dev.seoTitle || `${dev.name} — Developer Profile | Portugal Developments Review`
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '60px', padding: '48px 0', alignItems: 'start' }}>
          <article>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1px', border: '1px solid var(--border)', background: 'var(--border)', marginBottom: '40px' }}>
                {dev.foundedYear && (
                  <div style={{ background: 'var(--background)', padding: '16px' }}>
                    <dt style={{ fontSize: '11px', fontFamily: 'sans-serif', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '4px' }}>{d.foundedLabel}</dt>
                    <dd style={{ fontSize: '16px', fontWeight: 400, margin: 0 }}>{dev.foundedYear}</dd>
                  </div>
                )}
                {dev.headquartersCity && (
                  <div style={{ background: 'var(--background)', padding: '16px' }}>
                    <dt style={{ fontSize: '11px', fontFamily: 'sans-serif', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '4px' }}>{d.hqLabel}</dt>
                    <dd style={{ fontSize: '16px', fontWeight: 400, margin: 0 }}>{dev.headquartersCity}</dd>
                  </div>
                )}
                {dev.website && (
                  <div style={{ background: 'var(--background)', padding: '16px' }}>
                    <dt style={{ fontSize: '11px', fontFamily: 'sans-serif', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '4px' }}>{d.websiteLabel}</dt>
                    <dd style={{ margin: 0 }}>
                      <a
                        href={dev.website}
                        target="_blank"
            ...
            {dev.isViriatoClient && (
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: '16px', marginBottom: '40px' }}>
                <span style={{ fontSize: '12px', fontFamily: 'sans-serif', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--muted)' }}>
          ...
          <aside style={{ position: 'sticky', top: '80px' }}>
            <div style={{ border: '1px solid var(--border)', padding: '24px' }}>
              <Link
                href={`/${lang}/contact`}
                style={{ display: 'block', background: 'var(--foreground)', color: 'var(--background)', padding: '14px 24px', fontSize: '13px', fontFamily: 'sans-serif', letterSpacing: '0.06em', textTransform: 'uppercase', textDecoration: 'none', textAlign: 'center', marginBottom: '16px' }}
              >
                {d.contactCta}
              </Link>
              <p style={{ fontSize: '13px', color: 'var(--muted)', fontFamily: 'sans-serif', lineHeight: 1.6, margin: '0 0 16px' }}>
                {d.isYourCompany}
              </p>
              <Link
                href={`/${lang}/contact`}
      ...
      <section style={{ borderTop: '1px solid var(--border)', padding: '48px 0' }}>
        <div className="container-editorial">
          ...
          {(developments as any[]).length === 0 ? (
            <p style={{ fontSize: '15px', color: 'var(--muted)', fontFamily: 'sans-serif' }}>{d.noDevelopmentsByDeveloper}</p>
          ) : (
